...
func main() {
	...
	file, err := os.Open(fmt.Sprintf("%s.go", filename))
	if err != nil {
		log.Fatalf("Invalid filename: %s\n", filename)
	}
...
func argCheck() {
	if len(os.Args) == 2 {
		...
		log.Fatalln("No argument specified")
	}
}
...
func generateTestFile(packagename string, functions []string) {
	_, err := os.Stat(filename + "_test.go")
	if err != nil {
		generateNewTestFile(filename+"_test.go", packagename, functions)
	} else {
...
func overwriteOldTestFile(filename, packagename string, functions []string) {
	file, err := os.OpenFile(filename+"_test.go", os.O_RDWR, 0755)
	if err != nil {
